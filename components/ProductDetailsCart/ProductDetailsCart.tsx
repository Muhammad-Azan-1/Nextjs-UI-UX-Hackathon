...
const ProductDetailsCart = ({
  ...
  return (
    <>
      ...
      <div>
        <div className=" pt-4">
          <p className="text-[#151875] pb-2">
            <span className="font-[600]">Color:</span> {Selectedcolor}
          </p>
          <div className="flex gap-x-2 ">
            {colors.map((colors: string, index: number) => (
              <div
                key={index}
                className={`p-[2px] rounded-[50%]  ${Selectedcolor === colors ? "border-[1px] border-[#151875]" : ""}`}
              >
                <button
                  onClick={() => changeColor(colors)}
                  style={{ backgroundColor: colors }}
                  className={`w-[30px] p-1 h-[30px] rounded-[50%] border-[1px] `}
                ></button>
              </div>
            ))}
          </div>
