        ...
        <div className="w-full h-auto bg-[#F1F0FF] flex flex-col md:flex-row justify-center items-center py-4 mt-24 px-2 xl:px-0">
          ...
          >
            ...
            <div className="flex mt-6">
              <HomeIncrementButton stock={Product[16]?.stockLevel} id={Product[16]?.id} name={Product[16]?.name} price={Product[16]?.price} image={Product[16]?.imageUrl} colors={Product[16]?.colors} />
              <div className="ml-5">
                <p className="text-[14px] text-[#151875] font-josefin">
                  {Product[16]?.name}
                </p>
                <p className="text-[#151875] font-lato">Rs.{Product[16]?.price}</p>
              </div>
            </div>
          </div>
        </div>
