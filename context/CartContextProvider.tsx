...
const CartContextProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const [cartItems, setcartItems] = useState<Quantity>(() => {
        if (typeof window != 'undefined' && sessionStorage) {
            ...
            return storedAmount ? JSON.parse(storedAmount) : {};
        }
    });
    ...
    const setIncrement = useCallback((stock: number, id: number, name: string, price: number, image: string, color: string , slug?:string) => {
        setcartItems((prev: Quantity) => {
            setId(id)
            const key = `${id}-${color}`
